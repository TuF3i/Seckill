...
service UserSvr {
    void RegisterUser(1: string email, 2: string password) // 注册
    JWTToken Login(1: string email, 2: string password) // 登录
    void Logout(1: string uid) // 推出登录
    string RefreshAccessToken(1: JWTClaims claims) // 刷新accessToken
